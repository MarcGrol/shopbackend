...
func TestCheckoutService(t *testing.T) {
	...
	t.Run("Create checkout with api-key", func(t *testing.T) {
		...
		// given
		vault.EXPECT().Get(gomock.Any(), oauthvault.CurrentToken+"_"+"adyen").Return(oauthvault.Token{}, false, nil)
	...
	t.Run("Create checkout with access-token", func(t *testing.T) {
		...
		// given
		vault.EXPECT().Get(gomock.Any(), oauthvault.CurrentToken+"_"+"adyen").Return(oauthvault.Token{
			ProviderName: "adyen",
...
`, got)
		...
		assert.Equal(t, "AUTHORISATION=true", checkout.CheckoutStatusDetails)
	})
}
